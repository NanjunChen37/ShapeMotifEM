...
#' @export SMEM
SMEM <- function(dna_shape_data, filename = "SMEM", peakCount = 20, motifLength = 12, motifCount = 2, replicates = 50, tolerance = 0.000001) {
  ...
  # Function of the EM algorithm for single shape feature
  EM_single <- function(inputData, peakCount, K, inputMotifLength, tol) {
    ...
    while (currentTol > tol && iter < maxIterations) {
      ...
      for (k in 1:K) {
        for (i in 1:peakCount) {
          prior[k, i] = 0
          posterior[[k]][[i]] = rep(0, (length(inputData[[i]]) - inputMotifLength + 1))
          for (j in 1:(length(inputData[[i]]) - inputMotifLength + 1)) {
            tempMotif = inputData[[i]][j:(j + inputMotifLength - 1)]
            posterior[[k]][[i]][j] = prod(dnorm(tempMotif, mean = currentTimeSeriesMotifs[[k]], sd = BGsd)) / prod(dnorm(tempMotif, BGmeans, BGsd))
          }
          prior[k, i] = sum(posterior[[k]][[i]])
          prior[k, i] = prior[k, i] * pi[k]
          if (sum(posterior[[k]][[i]]) == 0 || is.na(sum(posterior[[k]][[i]]))) {
            posterior[[k]][[i]] = rep(1 / length(posterior[[k]][[i]]), length(posterior[[k]][[i]]))
          }
          posterior[[k]][[i]] = posterior[[k]][[i]] / sum(posterior[[k]][[i]])
        }
      }
      colSumMask = matrix(rep(colSums(prior), K), nrow = K, byrow = TRUE)
      prior = prior / colSumMask
      prior[is.na(colSumMask) | colSumMask == 0] = 1 / K
      ...
      for (k in 1:K) {
        ...
        for (i in 1:peakCount) {
          currentMotif = matrix(0, nrow = shapeFeatureCount, ncol = inputMotifLength)
          for (j in 1:(length(inputData[[i]]) - inputMotifLength + 1)) {
            tempMotif = inputData[[i]][j:(j + inputMotifLength - 1)]
            currentMotif = currentMotif + tempMotif * posterior[[k]][[i]][j]
          }
    ...
    for (k in 1:K) {
      for (i in 1:peakCount) {
        finalLocationsInitial[k, i] = which.max(posterior[[k]][[i]])
      }
    }
  ...
  # Function of the EM algorithm for multiple shape features
  EM_mod <- function(inputData, peakCount, K, inputMotifLength, tol) {
    ...
    while (currentTol > tol && iter < maxIterations) {
      ...
      for (k in 1:K) {
        for (i in 1:peakCount) {
          prior[k, i] = 0
          posterior[[k]][[i]] = rep(0, (ncol(inputData[[i]]) - inputMotifLength + 1))
  ...
  if (NumOfSDs > 1){
    ...
    while ((peakCount * cycleCount) < lenGrSet) {
      ...
      outputFinalLocation = paste("Location_", filename, "_Top_", lenGrSet, "_", startPoint, "_to_", endPoint, "EM", ".csv", sep = "")
      fileConnFinalLocation = file(outputFinalLocation, open = "wt")

      # Output evaluation metrics file
      outputFileName = paste("Experiment_", filename, "_Top_", lenGrSet, "_", startPoint, "_to_", endPoint, "EM", ".csv", sep = "")
      ...
      for (r in 1:replicates) {
        ...
        possibleError <- tryCatch({
          ...
          # print(paste(peakCount, peakLength, timeSeriesMotifCount, motifLength, NumOfSDs, timeTaken
          #             ))
  ...
  LocationArrayFilename = paste("Location_", filename, "_Top_", lenGrSet, "_1_to_", endPoint, "EM", ".csv", sep = "")
...
# em_test = SMEM(gr_test$dna_shape_data, filename = "SMEM", peakCount = 20, motifLength = 10, motifCount = 2, replicates = 5, tolerance = 0.000001)
