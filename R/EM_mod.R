EM_mod <- function(inputData, peakCount, K, inputMotifLength, tol) {
  ...
  # peakNumber = 1
  # for (x in 1:2) {
  #   experimentData[[peakNumber]] <- predDataTrans[, , x]
  ...
  #initialTimeSeriesMotifsSD = list()
  for (k in 1:K) {
    randomSeqIndex = sample(1:length(inputData), 1)
    randomSeqPosition = sample(1:(ncol(inputData[[randomSeqIndex]]) - inputMotifLength +
                                    1), 1)
  ...
  #currentTimeSeriesMotifsSD = initialTimeSeriesMotifsSD
  ...
  while (currentTol > tol && iter < maxIterations) {
    ...
    for (k in 1:K) {
      for (i in 1:peakCount) {
        prior[k, i] = 0
        posterior[[k]][[i]] = rep(0, (ncol(inputData[[i]]) - inputMotifLength +
                                        1))
        for (j in 1:(ncol(inputData[[i]]) - inputMotifLength + 1)) {
          tempMotif = inputData[[i]][, j:(j + inputMotifLength - 1)]
          posterior[[k]][[i]][j] = prod(dnorm(tempMotif, mean = currentTimeSeriesMotifs[[k]], sd =
                                                BGsd)) / prod(dnorm(tempMotif, BGmeans, BGsd))
        }
        prior[k, i] = sum(posterior[[k]][[i]])
        prior[k, i] = prior[k, i] * pi[k] # *dnorm(inputData[[i]],BGmeans,BGsd)
        if (sum(posterior[[k]][[i]]) == 0 ||
            is.na(sum(posterior[[k]][[i]]))) {
          posterior[[k]][[i]] = rep(1 / length(posterior[[k]][[i]]), length(posterior[[k]][[i]]))  # Numerical Underflow Handling
        }
