#' DNA shape motif discovery with generalized Gibbs sampling method.
#'
...
#' @export SM_Gibbs
SM_Gibbs <- function(dna_shape_data, filename = "file", peakCount = 20, motifLength = 12, motifCount = 2, replicates = 50, tolerance = 0.000001) {
  ...
  # Function of the generralized Gbbis algorithm for multiple shape features
  Gibbs_mod <- function(inputData, peakCount, K, inputMotifLength, tol){
  ...
  if (NumOfSDs > 1){
    ...
    while ((peakCount * cycleCount) < lenGrSet) {
      ...
      for (r in 1:replicates) {
        ...
        possibleError <- tryCatch({
          start_time <- Sys.time()
          results = Gibbs_mod(experimentData, peakCount, timeSeriesMotifCount, motifLength, tol)
    ...
    stop("Number of shape features must be a integer more than 1")
  }

  # Merge all the location results and output csv
  finalLocationArray = do.call(cbind, finalLocationArray)[, 1:lenGrSet]
  LocationArrayFilename = paste("Location_", filename, "_Top_", lenGrSet, "_1_to_", endPoint, "Gibbs", ".csv", sep = "")
...
# test = test$value
#
# em_test = SM_Gibbs(test, filename = "file", peakCount = 2, motifLength = 10, motifCount = 2, replicates = 5, tolerance = 0.000001)

# dna_shape_data = predDataTrans
# filename = locationResultsName
# peakCount = 100
# motifLength = 20
# motifCount = 2
# replicates = 500
