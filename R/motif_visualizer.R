...
#' @param shapeIndex the index of shape feature to process, 1 to 5 indicate feature "MGW", "HelT", "ProT", "Roll", and "EP" respectively. "all" for mutiple shape alignment.
...
motif_visualizer <- function(motif_data_frame, motif_shape_data, motifFrequency = 0.6, shapeIndex = NULL, motifLength = NULL,
  ...
  # Function of getting pearson aligned data frame with all shape features
  all_pearson_aligned_df <- function(alignedShapeAll, alignedShapeAll_features) {

    # Draw motif line data frame
    shapedfmSet_all = c()
    for (s in 1:nrow(alignedShapeAll[[1]][[1]])){
      shapedfmSet = c()
      for (a in 1:length(alignedShapeAll_features[[s]])){
        alignedShape = alignedShapeAll_features[[s]][[a]]
        colindex = c()
        for (i in 1:nrow(alignedShape)){
          colindex[i] = paste("Motif", i, sep=" ")
        }
        ...
        dimnames = c("label", colindex)
        shapeMatrix <-
          matrix(nrow = nrow(alignedShape) + 1, ncol = ncol(alignedShape))
        shapeMatrix[1,] = c(1:ncol(alignedShape))
        for (i in 1:nrow(alignedShape)) {
          shapeMatrix[(i + 1),] = alignedShape[i,]
        }
        shapedf = as.data.frame(t(shapeMatrix))
        names(shapedf) <- dimnames
        shapedfm <- melt(shapedf,id="label")
        colnames(shapedfm) = c("label", "Legend", "value")

        shapedfmSet[[a]] = shapedfm
      }
      shapedfmSet_all[[s]] = shapedfmSet
    }
    #####drawMotifAreaDF####
    areadfmnSet_all = c()
    for (s in 1:nrow(alignedShapeAll[[1]][[1]])){
      ...
      for (a in 1:length(alignedShapeAll_features[[s]])){
        ...
        areadfmn <- data.frame(label = label,
                               maxv = maxv2,
                               minv = minv2,
                               meanv = meanv2)
        areadfmnSet[[a]] = areadfmn
      }
      areadfmnSet_all[[s]] = areadfmnSet
    }

    return(list("motif_shape_df_melt" = shapedfmSet_all, "motif_area_df_melt" = areadfmnSet_all))
  }
  ...
  if (continue_alignment == "1") {
    ...
    if (shapeIndex == "all"){
      ...
      align_results = motif_aligner_all_pearson(shape_data, motifLength, threshold, input_form, sequence_data)
      ...
      alignedShapeAll_features = c()
      for (s in 1:nrow(alignedShapeAll[[1]][[1]])){
        alignedShapeAll_t = c()
        for (i in 1:length(alignedShapeAll)){
          aligned_shape <- lapply(alignedShapeAll[[i]], function(x) x[s, ])
          aligned_shape_matrix = matrix(unlist(aligned_shape), nrow = length(alignedShapeAll[[i]]), ncol = motifLength, byrow = TRUE)
          alignedShapeAll_t[[i]] = aligned_shape_matrix
        }
        alignedShapeAll_features[[s]] = alignedShapeAll_t
      }

      align_df = all_pearson_aligned_df(alignedShapeAll, alignedShapeAll_features)
...
# motif_data_frame = fa_mege_test$motif_data_frame
# motif_shape_data = fa_mege_test$motif_shape_data
# #
# test_v = motif_visualizer(motif_data_frame, motif_shape_data, motifFrequency = 0.4, shapeIndex = "all",
#                           motifLength = 8, align_method = "pearson", ref_number = 2)
# #
# motifFrequency = 0.7
# ref_number = 1
# shapeIndex = 1
